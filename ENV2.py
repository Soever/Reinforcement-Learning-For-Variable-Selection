...
class FSEnv():
    def __init__(self, df_class, state_size, action_size, invalid_action_reward=0, min_score=0, min_step_size=1,
                 max_stop_step=None):
    ...
    def is_valid_action(self, action, state):
        ...
        if param_type < 2:  # 对于 t 或 tao 的操作
            ...
            if c_value == 0:
                ...
                if is_increment:
                    if state[param_type * self.feature_num + feature_index] + self.min_step_size > self.ub[
                        param_type * self.feature_num + feature_index]:
                        return False
                else:
                    if state[param_type * self.feature_num + feature_index] - self.min_step_size < self.lb[
                        param_type * self.feature_num + feature_index]:
                        return False
