...
class FSEnv():
    ...
    def init_state_random(self):
        ...
        random_state = np.array(random_vector)
        self.daXY_history = np.array(self.df_class.orign_df.copy().dropna())
        filtervalue = random_state [:self.feature_num]
        delayvalue = random_state [self.feature_num:2 * self.feature_num]
        varSelect = random_state [2 * self.feature_num:].astype(bool)
        originDF = self.df_class.orign_df.copy()

        # 处理滞后
        df = dataShift(originDF, delayvalue)
        # 处理滤波
        X, y = dataFilter(df, filtervalue)
        self.dataX_history = X
        self.dataY_history = y
        self.daXY_history[:,:-1]=X
        self.daXY_history[:,-1] =y

        if varSelect.sum() >0 :
            X_varSelect = self.dataX_history[:, varSelect]
            bestR2, latent_var_num = calculate_best_R2_Parrel(X_varSelect, y)
            self.best_R2 = bestR2
            self.best_state = random_state
            self.acc_old = self.best_R2
            self.acc_new = 0
        else:
            self.best_R2 = 0
            self.best_state = None
