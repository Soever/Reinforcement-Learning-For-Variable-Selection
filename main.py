...
def PPO_learn(df_class,directory):
    state_size = df_class.feature_num * 3  # 每个变量有3个参数
    action_size = df_class.feature_num * 6  # 每个参数有两个动作，增或减
    actor_lr = 1e-3
    critic_lr = 1e-2
    ...
    env = FSEnv(df_class=df_class, state_size=state_size, action_size=action_size,
                ...
                max_stop_step=20,  # 最大停滞步数 智能体n步都不提升时停止
                )
    # env.seed(0)
...
def save_result(return_list, r2_list, state_list,directory):
...
if __name__ == '__main__':
    ...
    if not os.path.exists(directory):
        os.makedirs(directory)
    logging.basicConfig(filename=directory+'/debuglog.log', level=logging.DEBUG)
