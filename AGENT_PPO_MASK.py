...
class PPO_MASK:
    ...
    def update(self, transition_dict):
        ...
        action_mask = torch.tensor(np.array(transition_dict['masks']), dtype=torch.float).to(self.device)
        ...
        old_log_probs = torch.log(self.actor(states, action_mask).gather(1, actions)).detach()


        for _ in range(self.epochs):
            log_probs = torch.log(self.actor(states, action_mask).gather(1, actions))
